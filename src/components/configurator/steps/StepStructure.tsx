        ...
        {STRUCTURE_OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              aria-pressed={sel}
              className={`relative p-4 rounded-xl border text-left transition-all cursor-pointer flex flex-col gap-2 min-h-[108px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-yellow ${
            ...
            >
              ...
              {opt.premium && (
                <span className="self-start text-[7px] font-mono font-black uppercase px-1.5 py-0.5 rounded bg-zinc-900 text-brand-yellow border border-brand-yellow/30">
                  Premium
                </span>
              )}
              <p className={`text-xs font-black uppercase tracking-tight ${sel ? "text-white" : "text-zinc-300"}`}>{opt.name}</p>
              <p className="text-[10px] text-zinc-600 leading-tight">{opt.shortDescription}</p>
            </button>
          );
        })}
      </div>
